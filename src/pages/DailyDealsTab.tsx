...
export default function DailyDealsTab({ currentUser, goals, onRefresh }: DailyDealsTabProps) {
  ...
  const fetchAllUsers = async () => {
    const { data } = await supabase
      .from('profiles').select('id, name').eq('active', true).order('name');
    if (data) setAllUsers(data.map((r: any) => ({ id: r.id, name: r.name })));
  };
  ...
  // Leaderboard
  const leaderboard: RepStat[] = allUsers.map(u => {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Deals Today</p>
              <p className="text-3xl font-bold text-green-400 leading-none">{dealCount}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Total Amount</p>
              <p className="text-xl font-bold text-green-400 leading-none">{formatCurrency(totalAmount)}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Team Goal</p>
              <div className="flex items-baseline gap-1 mb-2">
                <p className="text-xl font-bold text-blue-400 leading-none">{dealCount}</p>
                <p className="text-sm text-gray-500">/ {goals.team}</p>
              </div>
              <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${goalPct}%` }} />
              </div>
              <p className="text-xs text-gray-500 mt-1">{goalPct.toFixed(0)}% complete</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Active Reps</p>
          ...
          <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
            ...
            {showForm && (
              <div className="p-5">
                {formError && (
                  <div className="bg-red-900 border border-red-700 text-red-200 px-3 py-2 rounded text-sm mb-4">{formError}</div>
                )}
          ...
          <div className="divide-y divide-gray-700">
            {leaderboard.length === 0 ? (
              ...
              leaderboard.map((rep, idx) => {
                ...
                return (
                  <div key={rep.id}
                    className={`flex items-center gap-3 px-4 py-3 ${isMe ? 'bg-blue-900 bg-opacity-25' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">
                        {rep.totalAmount > 0 ? formatCurrency(rep.totalAmount) : <span className="italic">no entries yet</span>}
                      </p>
                    </div>
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-700 flex items-center justify-between">
          <p className="text-sm font-semibold text-gray-200">
            Today's entries
            <span className="ml-2 font-normal text-gray-400">{todayDeals.length} {todayDeals.length === 1 ? 'deal' : 'deals'}</span>
          </p>
          <p className="text-xs text-gray-500">Expand a row to add notes</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-700 bg-gray-800">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Added by</th>
                  <th className="w-10 px-4 py-3"></th>
                </tr>
              </thead>
                    ...
                    <>
                      ...
                      {isExpanded && (
                        <tr key={`${deal.id}-expanded`}>
                          <td colSpan={10} className="px-4 py-4 pl-14 bg-gray-750 border-b border-gray-700">
      ...
      {showHistory && (
        <div
          className="fixed inset-0 bg-black bg-opacity-70 flex items-start justify-center pt-16 z-50 px-4"
          onClick={() => { setShowHistory(false); setSelectedDate(null); setSelectedDateDeals([]); }}
        >
          <div
            className="bg-gray-800 rounded-xl border border-gray-600 w-full max-w-2xl overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
              ...
              <button
                onClick={() => { setShowHistory(false); setSelectedDate(null); setSelectedDateDeals([]); }}
                className="text-gray-400 hover:text-gray-200 text-2xl font-light leading-none"
              >
                &times;
              </button>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="flex gap-5 mb-4 pb-4 border-b border-gray-700">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">Has entries — click to view</span>
                </div>
                <div className="flex items-center gap-2">
              ...
              {selectedDate && (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-semibold text-gray-200">
                      {formatDateLabel(selectedDate)} — {selectedDateDeals.length} {selectedDateDeals.length === 1 ? 'deal' : 'deals'}
                    </p>
                    <button
                      ...
                      <tbody className="divide-y divide-gray-700 bg-gray-800">
                        {selectedDateDeals.map(deal => (
                          <tr key={deal.id} className="hover:bg-gray-750">
                            ...
                            <td className="px-3 py-2.5">
                              <span className={`px-2 py-0.5 rounded-full text-xs border ${getFuStatusStyle(deal.fuStatus)}`}>
                                {deal.fuStatus}
                              </span>
                            </td>
