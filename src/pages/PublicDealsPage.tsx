...
export default function PublicDealsPage() {
  ...
  const handleUpdateCombined = async (entry: CombinedEntry) => {
    try {
      if (entry.source === 'manual') {
        ...
        const lockStart = manualRow?.createdAt;
  ...
  return (
    <div className="min-h-screen bg-dss-canvas">
      <header className="bg-dss-surface border-b border-dss-border sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-dss-accent">DSS Portal</h1>
            <span className="text-xs text-dss-muted bg-dss-canvas px-2.5 py-1 rounded-full border border-dss-border">Daily Deals</span>
          </div>
          <span className="text-xs text-dss-muted">{formatDateLabel(today)}</span>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-4 items-start">
          <div className="space-y-4">
                ...
                <div className="relative flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="mt-2 text-xl font-bold text-cyan-800 leading-none">
                      {totalDealCount} <span className="text-sm font-normal text-dss-muted">/ {teamGoal}</span>
                    </p>
                  </div>
            ...
            <div className="bg-dss-surface rounded-dss border border-dss-border overflow-hidden shadow-sm">
              <div className="px-5 py-4 border-b border-dss-border flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-dss-ink">Log a deal</h2>
              </div>
              <div className="p-5 space-y-4">
                ...
                {!linkedCall && (
                  <div className="bg-dss-canvas border border-dss-border rounded-dss p-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={handleSearch} disabled={searching}
                        className="px-4 py-2 bg-dss-navy-soft hover:bg-dss-navy disabled:bg-dss-canvas text-white rounded-dss-sm text-sm font-medium transition">
                        ...
                        {searchResults.map((call: any) => (
                          <div key={call.id} className="flex items-center justify-between px-3 py-2 bg-dss-canvas rounded-dss-sm border border-dss-border">
                            <div className="flex items-center gap-3 flex-wrap">
                              <span className="text-sm text-dss-accent font-medium">{call.application_id}</span>
                              <button
                              ...
                              <span className="text-xs text-dss-muted">{formatCurrency(parseAmount(call.buyer_final))}</span>
                              {call.customer_full_name && <span className="text-xs text-dss-muted italic">{call.customer_full_name}</span>}
                            </div>
                            <button onClick={() => handleSelectCall(call)}
                              className="text-xs px-3 py-1 bg-dss-navy-soft hover:bg-dss-navy text-white rounded-dss-sm transition flex-shrink-0">Use this app →</button>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                {appIdConflicts.length > 0 && !linkedCall && (
                  <div className="rounded-dss border border-amber-700/70 bg-amber-950/40 px-4 py-3">
                    ...
                    <div className="mt-2 space-y-1">
                      {appIdConflicts.map((call: any) => (
                        <div key={call.id} className="flex items-center justify-between px-3 py-2 bg-dss-surface/80 rounded-dss-sm border border-amber-800/50">
                          <div className="flex items-center gap-3 flex-wrap">
                            <span className="text-sm text-dss-accent font-medium">{call.application_id}</span>
                            <span className="text-xs text-dss-ink/80">{call.dealer_name}</span>
                            <span className="text-xs text-dss-muted">{call.state}</span>
                            {call.customer_full_name && <span className="text-xs text-dss-muted italic">{call.customer_full_name}</span>}
                          </div>
                          <button onClick={() => handleSelectCall(call)}
                ...
                {checkingAppId && !linkedCall && form.appId.trim() && (
                  <p className="text-xs text-dss-muted">Checking App ID against Calls…</p>
                )}

                <div>
                  <label className="block text-xs text-dss-muted uppercase tracking-wider mb-1.5">Your name *</label>
                ...
                {linkedCall ? (
                  <div className="border border-dss-accent/30 rounded-dss-sm overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 bg-dss-accent-soft bg-opacity-20 border-b border-dss-navy-soft">
                      <div>
                        <span className="text-sm font-medium text-dss-accent">{linkedCall.application_id}</span>
                        <span className="text-xs text-dss-muted ml-2">·</span>
                        <span className="text-xs text-dss-ink/80 ml-2">{linkedCall.dealer_name}</span>
                        <span className="text-xs text-dss-muted ml-2">· {formatCurrency(parseAmount(linkedCall.buyer_final))} · {linkedCall.state}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="flex items-end">
                          <button onClick={handleSubmit} disabled={submitting}
                            className="w-full px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 disabled:from-gray-600 disabled:to-gray-600 text-white rounded-dss-sm text-sm font-medium transition shadow-sm">
                  ...
                  <>
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        ...
                        <input type="text" value={form.appId} onChange={e => setForm({ ...form, appId: e.target.value })}
                          ...
                          className={`w-full px-3 py-2 bg-dss-canvas border rounded-dss-sm text-sm text-dss-ink placeholder-dss-muted/70 focus:outline-none focus:ring-1 focus:ring-dss-accent/30 ${appIdConflicts.length > 0 ? 'border-amber-600' : 'border-dss-border'}`} />
                      </div>
                      <div>
                    ...
                    {softMatches.length > 0 && appIdConflicts.length === 0 && (
                      <div className="rounded-dss border border-sky-700/70 bg-sky-950/30 px-4 py-3">
                    ...
                    {checkingSoftMatch && appIdConflicts.length === 0 && form.dealerName.trim() && form.customerName.trim() && (
                      <p className="text-xs text-dss-muted">Checking dealer + customer for possible matches…</p>
                    )}
                    ...
                    <p className="text-xs text-dss-muted">Counts toward today's team goal. Resets at midnight.</p>
                  </>
                )}
              </div>
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-dss-ink">Today's entries</h2>
            <span className="text-xs text-dss-muted">{combinedEntries.length} total</span>
          </div>
          {loading ? (
            <div className="bg-dss-surface rounded-dss-sm border border-dss-border p-8 text-center text-dss-muted text-sm">Loading…</div>
          ) : combinedEntries.length === 0 ? (
            <div className="bg-dss-surface rounded-dss-sm border border-dss-border p-10 text-center">
              <p className="text-sm text-dss-muted">No deals logged yet today.</p>
              <p className="text-xs text-dss-muted mt-1">Be the first to log one above.</p>
            </div>
          ) : (
            <div className="bg-dss-surface rounded-dss border border-dss-border overflow-hidden shadow-sm">
                ...
                <table className="w-full" style={{ tableLayout: 'fixed' }}>
                  ...
                  <thead className="bg-dss-canvas">
                    <tr className="border-b border-dss-border">
                      ...
                      <th className="px-2 py-2 text-left text-xs font-medium text-dss-muted uppercase tracking-wider">Status</th>
                      <th className="px-2 py-2 text-left text-xs font-medium text-dss-muted uppercase tracking-wider">Source</th>
                      <th className="px-2 py-2 text-left text-xs font-medium text-dss-muted uppercase tracking-wider">By</th>
                      <th className="px-2 py-2"></th>
                      <th className="px-2 py-2"></th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <>
                          <tr key={entry.id}
                            ...
                            <td className="px-2 py-2 text-xs text-dss-accent font-medium truncate">{entry.appId}</td>

                            <td className="px-2 py-2" onClick={e => e.stopPropagation()}>
                              <button onClick={() => openDealerPopup(entry.dealerName)}
                                className="text-xs text-left hover:text-dss-accent transition text-dss-ink truncate block w-full"
                            ...
                            <td className="px-2 py-2">
                              <span className="text-xs text-dss-muted truncate block w-full" title={entry.customerName}>{entry.customerName || '—'}</span>
                            </td>
                            ...
                            <td className="px-2 py-2" onClick={e => e.stopPropagation()}>
                              {isEditing ? (
                                ...
                                <div className="flex items-center gap-1">
                                  <span className="text-xs font-semibold text-dss-ink">{formatCurrency(parseAmount(entry.amount))}</span>
                                  <button onClick={() => startEditing(entry)} className="text-dss-muted hover:text-dss-muted transition flex-shrink-0"><Edit2 className="w-2.5 h-2.5" /></button>
                                </div>
                              )}
                            </td>
                            ...
                            <td className="px-2 py-2" onClick={e => e.stopPropagation()}>
                              {isEditing && entry.source === 'manual'
                                ...
                                : <span className="px-1.5 py-0 bg-dss-canvas text-dss-ink/80 text-[10px] rounded border border-dss-border">{entry.state}</span>}
                            </td>
                            ...
                            <td className="px-2 py-2" onClick={e => e.stopPropagation()}>
                              <span className={`px-1.5 py-0 rounded text-[10px] font-medium ${entry.source === 'call' ? 'bg-dss-accent-soft text-dss-accent border border-dss-accent/30' : 'bg-dss-canvas text-dss-muted border border-dss-border'}`}>
                                {entry.source === 'call' ? 'Calls' : 'Manual'}
                              </span>
                            </td>

                            <td className="px-2 py-2">
                              <span className="text-xs text-dss-muted truncate block w-full">{entry.creditName}</span>
                            </td>

                            <td className="px-2 py-2 text-center" onClick={e => e.stopPropagation()}>
                              {entry.source === 'manual' && (
                                <button onClick={e => toggleNotes(e, entry.id)} title="Notes"
                                  className={`inline-flex items-center justify-center w-6 h-6 rounded transition ${notes.length > 0 ? 'bg-dss-navy-soft text-white hover:bg-dss-navy' : 'bg-violet-50 text-violet-800 hover:bg-violet-100'}`}>
                            ...
                            <td className="px-2 py-2" onClick={e => e.stopPropagation()}>
                              {isEditing ? (
                                <div className="flex items-center gap-1">
                                  <button onClick={() => handleUpdateCombined(entry)} className="text-dss-success hover:text-dss-success"><Check className="w-3.5 h-3.5" /></button>
                                  <button onClick={() => setEditingDeal(null)} className="text-dss-danger hover:text-dss-danger"><X className="w-3.5 h-3.5" /></button>
                                </div>
                          ...
                          {isExpanded && !isEditing && entry.source === 'manual' && (
                            <tr key={`${entry.id}-exp`}>
                              <td colSpan={11} className="px-3 py-2 pl-8 bg-dss-canvas">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-70 flex items-start justify-center pt-10 z-50 px-4"
            ...
            <div className="bg-dss-surface rounded-dss border border-dss-border w-full max-w-5xl overflow-hidden"
              ...
              <div className="overflow-x-auto max-h-[65vh] overflow-y-auto">
                {dealerPopupLoading ? (
                  <div className="p-8 text-center text-dss-muted text-sm">Loading calls…</div>
                ) : (
                  <table className="w-full" style={{ tableLayout: 'fixed' }}>
                    <colgroup>
                    ...
                    {filteredDealerPopupCalls.length === 0 ? (
                        <tr><td colSpan={10} className="px-4 py-8 text-center text-sm text-dss-muted">No active calls found for this dealer</td></tr>
                            ...
                            <tr key={call.id}
                              ...
                              <td className="px-2 py-2">
                                <div className="flex items-center gap-1 flex-wrap">
                                  <span className="text-xs text-dss-accent font-medium truncate">{call.application_id}</span>
                              ...
                              <td className="px-2 py-2">
                                {call.customer_full_name
                                  ? <span className="text-xs text-dss-muted truncate block" title={call.customer_full_name}>{call.customer_full_name}</span>
                              ...
                              <td className="px-2 py-2 text-xs text-dss-muted whitespace-nowrap">{call.submitted_date}</td>
